...
module x_end_motor_base() {
    x_end_base();
    translate(v=[-15,31,26.5]) cube(size = [17,46,53], center = true);
}
...
module _long_hole(d,h,l) {
    hull() {
        translate([0,-l/2,0]) cylinder(d=d,h=h);
        translate([0,l/2,0]) cylinder(d=d,h=h);
    }
}

module x_end_motor_holes(brass_nut=false){
    x_end_holes(brass_nut=brass_nut);

    // Position to place
    translate(v=[-1,32,30.25]) {
        // Belt hole
        translate(v=[-14,-7,0]) cube(size = [10,30,22], center = true);

        // Motor mounting holes
        translate(v=[20,-15.5,-15.5]) rotate(a=[0,-90,0]) _long_hole(3.6,70,3,$fn=30);
        translate(v=[1,-15.5,-15.5]) rotate(a=[0,-90,0]) _long_hole(6.2,10,3,$fn=30);
        ...
        translate(v=[20,15.5,-15.5]) rotate(a=[0,-90,0]) rotate(a=[0,0,-45]) _long_hole(3.6,70,3,$fn=30);
        translate(v=[1,15.5,-15.5]) rotate(a=[0,-90,0]) rotate(a=[0,0,-45]) _long_hole(6.2,10,3,$fn=30);

        // Motor adjustment hole
        translate(v=[-14,-4,-35]) rotate([-45,0,0]) cylinder(h = 27, r=1.8, $fn=30);
        translate(v=[-14,-4,-35]) rotate([-45,0,0]) cylinder(h = 12, r=3.1, $fn=30);

        // Motor adjustment nut hole
        translate(v=[-14,7,-24]) rotate([-45,0,0]) hull() {
            translate([-10,0,0]) cylinder(h = 2.5, r = 3.45, $fn=6);
            cylinder(h = 2.5, r = 3.45, $fn=6);
        }

        // Material saving cutout 
        translate(v=[-10,12,11]) cube(size = [60,42,42], center = true);

        // Material saving cutout
        translate(v=[-10,39,-31]) rotate(a=[45,0,0])  cube(size = [60,42,42], center = true);
    }
}
...
// Final part
module x_end_motor(brass_nut=false){
    difference(){
        union(){
            x_end_motor_base();
        ...
        x_end_motor_holes(brass_nut=brass_nut);

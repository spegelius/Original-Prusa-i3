...
module x_end_holes(brass_nut=false) {
    ...
    if(brass_nut) {
        ...
        translate(v=[0,-17, -0.1]) cylinder(h = 0.5, r1 = 5+0.8,r2 = 5.2, $fn = 25);

        // Screw holes for TR nut
        translate(v=[0,-17, 0]) rotate([0, 0, -135]) translate([0, 16/2, -1]) cylinder(h = 10, r = 1.8, $fn=25);
        translate(v=[0,-17, 0]) rotate([0, 0, -135]) translate([0, -16/2, -1]) cylinder(h = 10, r = 1.8, $fn=25);

        // Nut traps for TR nut screws
        translate(v=[0,-17, 0]) rotate([0, 0, -135]) translate([0, 16/2, 6]) rotate([0, 0, 0])cylinder(h = 3, r = 3.45, $fn=6);

        translate(v=[0,-17, 0]) rotate([0, 0, -135]) translate([0, -16/2, 6]) rotate([0, 0, 30])cylinder(h = 3, r = 3.2, $fn=6);

        translate([-4.5,-18.2,6]) rotate([0,0,30]) cube([5,5,3]);
        translate([-0,-17.2,6]) rotate([0,0,60]) cube([5,8,3]);
    } else {
        // Hole for the nut
...
// Final prototype
module x_end_plain(brass_nut=false){
    difference(){
        x_end_base();
...
//x_end_plain(brass_nut=false);
x_end_plain(brass_nut=true);
...
module brass_nut() {
    difference() {
        union() {
            cylinder(d=22,h=4.8,$fn=30);
            translate([0,0,1.8]) cylinder(d=10,h=15.5,$fn=20);
        }
        translate([0,0,-2]) cylinder(d=8,h=20,$fn=20);
        for (i=[0:3]) {
            rotate([0,0,i*360/4]) translate([16/2,0,-0.1]) cylinder(d=3.5,h=5,$fn=10);
        }
    }
}

%translate([0,-17,-4.8]) rotate([0,0,45]) brass_nut();